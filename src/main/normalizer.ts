...
const localFfmpeg = ffmpegPath?.replace('app.asar', 'app.asar.unpacked');
...
class NormalizationSetting {
  ...
  constructor({ base, min, max }: { base: number; min: number; max: number }) {
    this.min = min;
    this.max = max;
    this.base = base;
  }
...
class Validator {
  public validate({ name, value }: { name: string; value: any }) {
    if (this[name]) {
      if (this[name].isValid(Number(value))) {
        ...
        logger.log(
          `Loudness parameter validator:: ${name} is not in range setting default ${this[name].base}.`
        );
        return this[name].base;
      }
    } else {
      logger.log(
        `Loudness parameter validator:: ${name} is not defined in current normalization method.`
      );
      return null;
    }
  }
...
class LoudnessFactory {
  public static build({ normalization, target }): Loudness {
    const validator = LoudnessFactory.buildValidator(normalization);
    const loudness = new Loudness(target, validator);
    return loudness;
  }
...
class Command {
  ...
  execute({ success, fail }: { success: Function; fail: Function }) {
    this.state = 'progress';
...
class Normalizer {
  public static validate({ input, output, loudness, ...rest }) {
    return new Promise((resolve, reject) => {
      loudness = LoudnessFactory.build(loudness);
      return this.fileHasAudio(input).then((hasAudio) => {
        if (hasAudio) {
          return resolve({
            input,
            output,
  ...
  private static addPadding(
    { input, output, originalDuration, ...rest },
    resolve,
    reject
  ) {
    ...
    const tempOutput = path.join(
      path.dirname(output),
      '__temporary.' + basename
    );

    const command = CommandFactory.addPadding(input, tempOutput);
    command.execute({
      success: ({ stderr }: ChildProcessSuccessMessage) => {
        if (stderr) {
          logger.error(stderr);
        }
  ...
  private static removePadding(
    { input, output, originalDuration, temporaryFile, ...rest },
    resolve,
    reject
  ) {
    const command = CommandFactory.removePadding(
      input,
      output,
      originalDuration,
      temporaryFile
  ...
  public static pad({ input, ...rest }) {
    return new Promise((resolve, reject) => {
      const command = CommandFactory.getDuration(input);
      command.execute({
        success: ({ stderr, processed }: ChildProcessSuccessMessage) => {
          if (stderr) {
            logger.error(stderr);
          }
          ...
          if (processed < 3) {
            Normalizer.addPadding(
              { input, originalDuration: processed, ...rest } as any,
              resolve,
              reject
            );
          } else {
  ...
  public static measure({ input, output, loudness, ...rest }) {
    return new Promise((resolve) => {
      const command = CommandFactory.measure({ input, loudness });
      command.execute({
        success: ({ stderr, processed }: ChildProcessSuccessMessage) => {
          if (stderr) {
            logger.error(stderr);
  ...
  public static change({ input, output, loudness, measured, padded, ...rest }) {
    return new Promise((resolve, reject) => {
      const command = CommandFactory.change({
        input,
        output,
        loudness,
        measured,
        ...
        success: () => {
          if (padded) {
            Normalizer.removePadding(
              { input, output, loudness, measured, padded, ...rest } as any,
              resolve,
              reject
            );
          } else {
...
class Parser {
  ...
  public static getMeasurements(stdout: string) {
    try {
      ...
      const measurements = JSON.parse(parsed);
      return measurements;
    } catch (error) {
      logger.error(error);
      return null;
   ...
   */
  static getDuration(stdout: string): number | null {
    try {
...
type NormalieType = 'ebuR128' | 'rms' | 'peak';

interface NormalizeInput {
  input: string;
  output: string;
  loudness: {
    normalization: NormalieType;
    target: {
      input_i: number;
      input_lra: number;
      input_tp: number;
...
export const normalize = (input: NormalizeInput) => {
  return new Promise((resolve, reject) => {
    ...
    return Normalizer.validate(input)
      ...
      .then((paddedInput) => {
        switch (normalization) {
          case 'ebuR128':
            return Normalizer.measure(paddedInput as any).then((measured) => {
