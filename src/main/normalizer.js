  ...
  Validator.prototype.validate = function (_a) {
    ...
    if (this[name]) {
      if (this[name].isValid(Number(value))) {
        ...
        logger.log(
          'Loudness parameter validator:: ' +
            name +
            ' is not in range setting default ' +
            this[name].base +
            '.'
        )
      ...
      logger.log(
        'Loudness parameter validator:: ' +
          name +
          ' is not defined in current normalization method.'
      )
      return null
    }
  }
...
var Loudness = /** @class */ (function () {
  function Loudness(_a, validator) {
    var input_i = _a.input_i,
      input_lra = _a.input_lra,
      input_tp = _a.input_tp,
      input_thresh = _a.input_thresh,
      target_offset = _a.target_offset
    if (validator) {
      this.input_i = validator.validate({ name: 'input_i', value: input_i })
      this.input_lra = validator.validate({
        name: 'input_lra',
        value: input_lra
      })
      this.input_tp = validator.validate({ name: 'input_tp', value: input_tp })
      this.input_thresh = validator.validate({
        name: 'input_thresh',
        value: input_thresh
      })
      this.target_offset = validator.validate({
        name: 'target_offset',
        value: target_offset
      })
    } else {
...
var LoudnessFactory = /** @class */ (function () {
  ...
  LoudnessFactory.build = function (_a) {
    var normalization = _a.normalization,
      target = _a.target
...
var CommandFactory = /** @class */ (function () {
  ...
  CommandFactory.measure = function (_a) {
    var input = _a.input,
      loudness = _a.loudness,
      rest = __rest(_a, ['input', 'loudness'])
    var command = ffmpeg_static_1.path
    var loudArg = 'loudnorm='
    loudArg += 'I=' + loudness.input_i + ':'
    loudArg += 'LRA=' + loudness.input_lra + ':'
    loudArg += 'tp=' + loudness.input_tp + ':'
    loudArg += 'print_format=json'
    var args = ['-hide_banner', '-i', input, '-af', loudArg, '-f', 'null', '-']
    return new Command({
      text: command,
      args: args,
  ...
  CommandFactory.change = function (_a) {
    var input = _a.input,
      output = _a.output,
      loudness = _a.loudness,
      measured = _a.measured
    var args = ['-hide_banner', '-i', input, '-af']
    var loudArg = 'loudnorm='
    loudArg += 'I=' + loudness.input_i + ':'
    ...
    if (measured) {
      loudArg += 'measured_I=' + measured.input_i + ':'
      loudArg += 'measured_LRA=' + measured.input_lra + ':'
      loudArg += 'measured_tp=' + measured.input_tp + ':'
      loudArg += 'measured_thresh=' + measured.input_thresh + ':'
      loudArg += 'offset=' + measured.target_offset
    }
    args.push(loudArg)
    args.push('-ar', '48k', '-y', output)
    return new Command({
      text: ffmpeg_static_1.path,
      args: args,
      processAfter: function () {}
    })
  }
  CommandFactory.getDuration = function (input) {
    var args = ['-hide_banner', '-i', input, '-f', 'null', '-']
    return new Command({
      text: ffmpeg_static_1.path,
      args: args,
      processAfter: function (_a) {
  ...
  CommandFactory.addPadding = function (input, output) {
    var args = ['-hide_banner', '-i', input, '-af', 'apad,atrim=0:3', '-y', output]
    return new Command({
      text: ffmpeg_static_1.path,
      args: args,
      processAfter: function () {}
  ...
  CommandFactory.removePadding = function (input, output, duration, temporaryFile) {
    var args = ['-hide_banner', '-i', input, '-af', 'apad,atrim=0:' + duration, '-y', output]
    return new Command({
      text: ffmpeg_static_1.path,
      args: args,
      processAfter: function () {
...
var Command = /** @class */ (function () {
  ...
  Command.prototype.execute = function (_a) {
    ...
    child.execFile(this.text, this.args, function (error, stdout, stderr) {
...
var Normalizer = /** @class */ (function () {
  ...
  Normalizer.validate = function (_a) {
    ...
    var input = _a.input,
      output = _a.output,
      loudness = _a.loudness,
      rest = __rest(_a, ['input', 'output', 'loudness'])
    return new Promise(function (resolve, reject) {
      loudness = LoudnessFactory.build(loudness)
      return _this.fileHasAudio(input).then(function (hasAudio) {
        if (hasAudio) {
          return resolve(__assign({ input: input, output: output, loudness: loudness }, rest))
        } else {
    ...
    return new Promise(function (resolve) {
      try {
        var command = new Command({
          text: ffprobe_static_1.path,
          args: ['-i', input, '-show_streams', '-select_streams', 'a', '-loglevel', 'error']
        })
        command.execute({
          success: function (_a) {
  ...
  Normalizer.addPadding = function (_a, resolve, reject) {
    var input = _a.input,
      output = _a.output,
      originalDuration = _a.originalDuration,
      rest = __rest(_a, ['input', 'output', 'originalDuration'])
  ...
  Normalizer.removePadding = function (_a, resolve, reject) {
    var input = _a.input,
      output = _a.output,
      originalDuration = _a.originalDuration,
      temporaryFile = _a.temporaryFile,
      rest = __rest(_a, ['input', 'output', 'originalDuration', 'temporaryFile'])
    ...
    return new Promise(function (resolve, reject) {
      ...
      command.execute({
        success: function (_a) {
          ...
          if (processed < 3) {
            Normalizer.addPadding(
              __assign({ input: input, originalDuration: processed }, rest),
              resolve,
              reject
            )
          } else {
  ...
  Normalizer.measure = function (_a) {
    var input = _a.input,
      ...
      rest = __rest(_a, ['input', 'output', 'loudness'])
    return new Promise(function (resolve, reject) {
      var command = CommandFactory.measure({
        input: input,
        output: output,
        loudness: loudness
      })
  ...
  Normalizer.change = function (_a) {
    var input = _a.input,
      ...
      measured = _a.measured,
      padded = _a.padded,
      rest = __rest(_a, ['input', 'output', 'loudness', 'measured', 'padded'])
    return new Promise(function (resolve, reject) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    return Normalizer.validate(input)
      .then(function (validated) {
        return Normalizer.pad(validated)
      })
      .then(function (paddedInput) {
        switch (normalization) {
          case 'ebuR128':
            return Normalizer.measure(paddedInput).then(function (measured) {
      ...
      ['catch'](function (error) {
        logger.error(error)
        return reject(
          __assign({ normalized: false, error: error.message ? error.message : error }, input)
        )
      })
  })
}
