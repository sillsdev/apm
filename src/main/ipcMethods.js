const { app, ipcMain, session, dialog, BrowserWindow, shell } = require('electron')
...
const { downloadFile, downloadStatus, downloadClose } = require('./downloadFile')
...
const ipcMethods = () => {
  ...
  ipcMain.handle('zipStreamOpen', async (event, fullPath) => {
    const index = generateUUID()
    zipStr.set(index, new StreamZip.async({ file: fullPath, nameEncoding: 'utf8' }))
    return index
  })
  ...
  ipcMain.handle('downloadFile', async (event, url, localFile) => {
    if (process.platform === 'win32') localFile = localFile.replace(/\//g, '\\')
    try {
