  ...
  organizationbible: [
    {
      ...
      relationships: {
        ...
        bible: { data: { type: 'bible', id: BIBLE_ID } },
      },
    },
  ],
});
...
describe('BurritoWrapper', () => {
  ...
  const mountBurritoWrapper = (
    initialState: ReturnType<typeof createInitialState>,
    initialEntry = `/burrito/${TEAM_ID}/wrapper`
  ) => {
  ...
  it('renders wrapper header and metadata built from team bible and contents', () => {
    const memory = createBurritoMockMemory(createWrapperDataset());
    mountBurritoWrapper(createInitialState(memory));

    cy.get('#BurritoScreen').should('exist');
    cy.contains('h1', 'Scripture Burrito - Wrapper').should('be.visible');
  ...
  it('Refresh clears stored wrapper and rebuilds metadata', () => {
    const memory = createBurritoMockMemory(createWrapperDataset());
    mountBurritoWrapper(createInitialState(memory));
