...
if (requestedSchema > 1 && schemaDefinition.models) {
  ...
  if (schemaDefinition.models.project?.attributes) {
...
const findMissingModels = (schema: RecordSchema, db: IDBDatabase): string[] => {
...
const saveOfflineProjectInfo = async (
  ...
  if (isElectron) {
    let recs = (await backup.query(q => q.findRecords('project'))) as ProjectD[]
    if (!Array.isArray(recs)) recs = [recs]
    const ops: RecordOperation[] = []
    const fingerprint = recs.length > 0 ? await getFingerprint() : ''
...
const updatePublicFlags = async (
  ...
  let pRecs = (await backup.query(q => q.findRecords('project'))) as ProjectD[]
...
const moveTranscriptionState = async (
  ...
  let pRecs = (await backup.query(q => q.findRecords('passage'))) as PassageD[]
  if (!Array.isArray(pRecs)) pRecs = [pRecs]
  let mediafiles = (await backup.query(q =>
  ...
  pRecs.forEach((r: PassageD) => {
    ...
    if (meds.length > 0) {
      const mediafile = meds[0] as MediaFileD
      if (
...
const fixVersion8 = async (backup: IndexedDBSource): Promise<void> => {
  const ops: RecordOperation[] = []
  const tb = new RecordTransformBuilder()
  ...
  let oRecs = (await backup.query(q => q.findRecords('user'))) as UserD[]
  if (!Array.isArray(oRecs)) oRecs = [oRecs]
  oRecs.forEach((r: UserD) => {
    if (
      (r.attributes?.digestPreference as unknown as boolean) === false ||
      (r.attributes?.digestPreference as unknown as boolean) === true
    ) {
      r.attributes = {
        ...r.attributes,
