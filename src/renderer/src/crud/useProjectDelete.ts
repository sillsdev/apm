...
export const useProjectDelete = () => {
  ...
  return async (projectid: string) => {
    ...
    // TT-6952: a project with no plan used to return here, so the project row
    // itself was never removed even though its offlineproject already had been.
    for (const planid of plans) {
      ...
      ops.push(t.removeRecord({ type: 'plan', id: planid }).toOperation());
    }
    ops.push(t.removeRecord({ type: 'project', id: projectid }).toOperation());
