...
export const getAllMediaRecs = (
  ...
  const versionId = (mediaVersion[0] as MediaFileD).id;
...
export const getMediaShared = (passageId: string, memory: Memory) => {
  ...
  return mediaRecs.length > 0
    ? (mediaRecs[0] as MediaFileD).attributes.readyToShare
      ? IMediaShare.Latest
...
export const getMediaEaf = (mediaRec: MediaFile, memory: Memory): string => {
  ...
  const encTranscript = transcription
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;')
  ...
  updateXml('@DATE', xmlDoc, DateTime.now().setLocale('en').toISO());
...
export const nameFromTemplate = (
  ...
  if (template === '') {
    const tmp = passageDefaultFilename(
      passRec,
      '',
      memory,
      undefined,
