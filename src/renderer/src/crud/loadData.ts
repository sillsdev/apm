...
export async function insertData(
  ...
  try {
    ...
    if (err.constructor.name !== 'RecordNotFoundException') {
      orbitError(orbitInfo(err, item.keys?.['remoteId'] || ''));
    }
  } finally {
    if (rec) {
...
async function processData(
  ...
  for (let ti = 0; ti < tables.length; ti += 1) {
    const t = tables[ti] as ResourceDocument;
    try {
      ...
      for (let ji = 0; ji < jsonData.length; ji += 1) {
        const item = jsonData[ji] as InitializedRecord;
        await insertData(
          item,
          memory,
          backup,
  ...
  try {
    ...
    do {
      ...
      if (transform.length > 0) {
        const r: OrgData = transform[0] as OrgData;
...
export async function LoadProjectData(
  ...
  try {
    ...
    do {
      ...
      if (transform.length > 0) {
        const r: ProjData = transform[0] as ProjData;
        const x = JSON.parse(r.attributes.json);
        tables.push(...(x.data as ResourceDocument[]));
        //checkIt();
        const len = (
        ...
        if (start === 0) {
          ...
          await backup.sync(() => oparray); // ?? This shouldn't be necessary
          await memory.sync(() => oparray);
          stat.snapEnd = new Date().getTime();
        }
        start = r.attributes.startnext;
        const comp = 15 + start * 4;
        if (comp < 100) setCompleted(comp);
      } else {
        //bail - never expect to be here
        start = -1;
