...
import { VwChecksum, ProjectD } from '../model';
...
export const useSanityCheck = (setLanguage: typeof actions.setLanguage) => {
  ...
  const stringToDateNum = (val: string) =>
    val
      ? DateTime.fromISO(val.endsWith('Z') ? val : val + 'Z', {
          zone: 'utc',
        }).toMillis()
      : 0;
  ...
  return async (projectId: string) => {
    ...
    const getTableRows = (table: string) => {
      ...
      const planId = related(project, 'plan');

      if (!planId) console.warn('planId', planId);
      const mediafileRows = () =>
        planId
          ? (
      ...
      switch (table) {
        ...
        case 'plan':
          return [findRecord(memory, 'plan', planId)];
        case 'project':
          return [project];
    ...
    const refreshTable = async (
      ...
      const dtSince = DateTime.fromMillis(since).toUTC().toISO() as string;
      try {
    ...
    if (!getGlobal('offline') && project?.keys?.remoteId) {
      ...
      for (let ix = 0; ix < tables.length; ix++) {
        await compareChecksum(checksums, tables[ix] as string);
      }
    }
  };
};
