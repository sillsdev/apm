...
export function useWaveSurferRegions(
  ...
  useEffect(() => {
    ...
    if (ws) {
      ...
      ws.on('region-dblclick', function () {
        if (!singleRegionOnly) {
          wsAddOrRemoveRegion();
        }
      });
      ws.drawer.on('dblclick', () => {
        if (!singleRegionOnly) {
          wsAddOrRemoveRegion();
        }
      });
  ...
  const mergeVerses = (autosegs: IRegion[]) => {
    ...
    const minLen = paramsRef.current?.segLenThreshold || 0.5;
    segs.forEach((s) => {
      const i = autosegs.findIndex((t: IRegion) => t.end > s.start);
      if (i < 0) {
        suggested.push(s);
      } else {
  ...
  const wsSplitRegion = (r: any, split: number) => {
    ...
    const newRegion = wavesurferRef.current.addRegion(region);
    let newSorted: string[] = [];
    if (r) {
      const sortedIds: string[] = getSortedIds();
      ...
      const firstRegion = wavesurferRef.current.addRegion(region);
  ...
  const getSortedIds = () => {
    ...
    const ids = regionIds();
    if (ids.length > 0) {
      let next: string | undefined = ids[0];
      while (next) {
        const r: any = wavesurferRef.current.regions.list[next];
        if (r) {
          sortedIds.push(next);
