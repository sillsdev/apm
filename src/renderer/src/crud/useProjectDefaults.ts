...
export const projDefSisterRecommendations = 'sisterRecommendations';
...
export const useProjectDefaults = () => {
  ...
  const getLocalDefault = (label: string, projId?: string) => {
    const str = localStorage.getItem(label + (projId ?? getGlobal('project')));
    if (str) {
      const ret = tryParseJSON(str);
