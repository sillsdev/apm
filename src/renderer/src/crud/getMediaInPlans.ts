...
export const getMediaInPlans = (
  ...
  if (onlyLatest) {
    if (onlyTypeId === VernacularTag) {
      ...
      const myMedia = media;
      const vernacularIds = getMediaInPlans(
        planids,
        mediaFiles,
        VernacularTag,
        true
...
export const getDownloadableMediaInPlans = (
  ...
  const ret = new Set<IPlanMedia>();
  planids.forEach((plan) => {
    const x = getDownloadableMediaInPlan(plan, memory);
...
export const getDownloadableMediaInPlan = (
  ...
  const sharedres = memory?.cache.query((q) =>
    q.findRecords('sharedresource')
  ) as SharedResourceD[];
  const plan = findRecord(memory, 'plan', planid) as PlanD;
  ...
  const media = getMediaInPlans([planid], mediaFiles, undefined, false);
  let ret: IPlanMedia[] = [];
  ...
  const plansecs = sections
    .filter((s) => related(s, 'plan') === plan)
    .map((s) => s.id);
  const planpsgs = passages.filter((p) =>
  ...
  const shared = sharedres.filter((sr) => srs.includes(sr.id));
  const sourcepsgs = shared.map((sr) => related(sr, 'passage') as string);
  const sharedMedia = mediaFiles.filter((m) =>
