...
// @ts-ignore we have no type file for the plugins
...
export function useWaveSurfer(
  ...
  useEffect(() => {
    function create(container: any, height: number) {
      const ws = createWaveSurfer(container, height, timelineContainer);
      ...
      ws.on('marker-click', function (marker: any) {
  ...
  const wsBlob = async () => {
    const backend = wavesurfer()?.backend as any;
  ...
  const wsInsertSilence = (seconds: number, position: number) => {
    ...
    if (originalBuffer) {
      const new_len = ((seconds / 1.0) * originalBuffer.sampleRate) >> 0;
      newBuffer = backend.ac.createBuffer(
        originalBuffer.numberOfChannels,
        new_len,
        originalBuffer.sampleRate
      );
  ...
  const wsRegionDelete = () => {
    ...
    const emptySegment = backend.ac.createBuffer(
      originalBuffer.numberOfChannels,
      new_len,
      originalBuffer.sampleRate
    );
