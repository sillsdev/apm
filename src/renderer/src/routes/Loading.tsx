...
export function Loading() {
  ...
  const fetchOrbitData = (props: action.IFetchOrbitData) =>
    dispatch(action.fetchOrbitData(props) as any);
  const setExpireAt = (expireAt: number) =>
    dispatch(action.setExpireAt(expireAt) as any);
  const doOrbitError = (error: IApiError) =>
    dispatch(action.doOrbitError(error) as any);
  const orbitComplete = () => dispatch(action.orbitComplete() as any);
  ...
  const InviteUser = async (newremote: JSONAPISource, userEmail: string) => {
    ...
    const allinvites: InvitationD[] = (await newremote.query((q) =>
    ...
    if (inviteId) {
      ...
      if (!invite) {
        try {
          ...
          if (
            (thisinvite[0] as InvitationD).attributes.email.toLowerCase() !==
  ...
  const loadDone = (projectId: string, planId: string) => {
