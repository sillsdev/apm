...
import { isPublishingTitle } from './passageTypeFromRef';
...
describe('RefRender', () => {
  ...
  it('should return the Book icon component', async () => {
    const { container } = render(
      <RefRender value={PassageTypeEnum.BOOK} pt={PassageTypeEnum.BOOK} />
    );
    await waitFor(() => expect(container.firstChild).not.toBe(null));
  ...
  it('should return the Alt Book icon component', async () => {
    const { container } = render(
      <RefRender value={PassageTypeEnum.ALTBOOK} pt={PassageTypeEnum.ALTBOOK} />
    );
    await waitFor(() => expect(container.firstChild).not.toBe(null));
  ...
  it('should return the a text fragment component', async () => {
    const { container } = render(
      <RefRender value={'1:1-4'} pt={PassageTypeEnum.PASSAGE} />
    );
    await waitFor(() => expect(container.firstChild).not.toBe(null));
