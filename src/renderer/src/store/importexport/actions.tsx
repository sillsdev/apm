  ...
  async (dispatch: any) => {
    ...
    if (!token || exportType === ExportType.ITFSYNC) {
      ...
      do {
        ...
        const projRec = getProjRec(projectid);
        const bodyFormData = new FormData();
        if (artifactType !== undefined) {
          const mediaList = mediaArtifacts({
            memory,
            projRec,
            artifactType,
            target,
...
export const importProjectToElectron =
  ...
  async (dispatch: any) => {
    ...
    async function getProjectFromFile(ser: JSONAPIDocumentSerializer) {
      const json = await importJson(ser, 'D_projects.json', filepath);
      let project: any;
      ...
      try {
        const rec = memory.cache.query((q) =>
    ...
    async function removeProject(ser: JSONAPIDocumentSerializer) {
      const rec = await getProjectFromFile(ser);
      ...
      if (projectsWithGroup.length === 1) {
        ...
        gms.forEach((gm) => {
          gmids.push(gm.id);
          const thisuser = related(gm, 'user');
          const groupsForUser = (
            memory.cache.query((q) =>
              q.findRecords('groupmembership')
            ) as GroupMembership[]
      ...
      const delOpArray: RecordOperation[] = [];
    ...
    async function saveToMemory(oparray: RecordOperation[], title: string) {
      try {
        ...
        reportError(orbitInfo(err as Error, title));
        throw err;
      }
    }
    async function saveToBackup(oparray: RecordOperation[], title: string) {
      if (!coordinator.activated) {
        try {
          return await backup.sync(() => oparray);
        } catch (err: unknown) {
          reportError(orbitInfo(err as Error, title));
          throw err;
        }
    ...
    async function processFile(
      ...
      const json = await importJson(ser, file);
