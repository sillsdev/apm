...
export function updateSegments(
  ...
  const json = tryParseJSON(allsegs);
  if (Array.isArray(json)) {
    const index = json.findIndex((j) => j['name'] === name);
    if (index >= 0) {
      if (fornamesegs) {
...
export function getSegments(name: string, segments: string): string {
  const json = tryParseJSON(segments);
  if (Array.isArray(json)) {
    const thisone = json.find((j) => j['name'] === name) as INamedRegion;
    if (thisone?.regionInfo) {
      const ri = thisone?.regionInfo;
...
export function getSortedRegions(segments: string): IRegion[] {
