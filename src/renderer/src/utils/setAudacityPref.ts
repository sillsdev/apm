...
export const audPrefsName = async (): Promise<string | undefined> => {
  if (await ipc?.isWindows()) {
    const appData = (await ipc?.appData()) as string;
...
const getAllPref = async (prefs: string): Promise<string | false> => {
  if (!(await ipc?.exists(prefs))) return false;
  return (await ipc?.read(prefs, 'utf-8')) as string;
};

const getAudPrefContent = async (inPrefs?: string): Promise<string | false> => {
  const prefs = inPrefs || (await audPrefsName());
  if (prefs === undefined) return false;
  return await getAllPref(prefs);
};

const getRecordChannels = (content: string): RegExpExecArray | null => {
  return /RecordChannels=([12])/.exec(content);
};

const getSaveLocation = (content: string): RegExpExecArray | null => {
  return /SaveLocationMode=([a-zA-Z0-9]*)/.exec(content);
};

const getExpLocation = (content: string): RegExpExecArray | null => {
  return /ExportLocationMode=([a-zA-Z0-9]*)/.exec(content);
};

const getTmpMatch = (content: string): RegExpExecArray | null => {
  return /\[Directories\]\r?\nTempDir=([^\r\n]*)\r?\n/.exec(content || '');
};

const getDirMatch = (content: string): RegExpExecArray | null => {
  return /\[Directories\/(Open|Save)\]\r?\nDefault=([^\r\n]*)/.exec(
    content || ''
  );
};

const getIoMatch = (content: string): RegExpExecArray | null => {
  return /\[Directories\/(Import|Export)\]\r?\nDefault=([^\r\n]*)/.exec(
    content || ''
  );
};

const getExpMatch = (content: string): RegExpExecArray | null => {
  return /\[ExportAudioDialog\]\r?\nFormat=([A-Z1-3]*)\r?\nDefaultPath=([^\r\n]*)/.exec(
    content || ''
  );
};

const setFolder = async (kind: string, target: string): Promise<string> => {
  const ln = (await ipc?.isWindows()) ? '\r\n' : '\n';
  return `[Directories/${kind}]${ln}Default=${target}${ln}LastUsed=${target}${ln}`;
};

export const resetAudacityPref = (prefs: string, data: string): void => {
  ipc?.write(prefs, data);
};

const changeValueTo1 = (data: string, m: RegExpExecArray): string => {
...
export const setAudacityPref = async (
  fullName: string
  ...
  if (expMatch) {
    const pathMatch = expMatch[2] as string;
    const expPos =
      passContent.indexOf(expMatch[0]) + expMatch[0].length - pathMatch.length;
    passContent =
      passContent.slice(0, expPos) +
      ioFolder +
      passContent.slice(expPos + pathMatch.length);
  } else {
