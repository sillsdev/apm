...
import { IProfileStrings, IState, ParatextMsgs } from '../model';
...
export const useHasParatext = (): { hasParatext: boolean; ptPath: string } => {
  ...
  const dispatch = useDispatch() as ThunkDispatch<IState, void, ParatextMsgs>;
  const getUserName = (
    token: string,
  ...
  useEffect(() => {
    if (isOffline)
      getParatextDataPath().then((val) => setPtPath(val || DefPtData));
