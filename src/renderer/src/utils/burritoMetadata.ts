...
const getProductName = (pkg: unknown): string | undefined => {
...
export const burritoMetadata = ({
  projRec,
  userRec,
}: IProps): Record<string, unknown> => {
  const created = DateTime.now().setLocale('en').toISO();
  const meta = {
    meta: {
      version: '0.3.1',
      category: 'source',
      generator: {
        softwareName:
          getProductName(packageJson) ?? packageJson.name ?? 'unknown',
