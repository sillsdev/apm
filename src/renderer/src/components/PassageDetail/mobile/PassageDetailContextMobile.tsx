...
const initState = {
  ...
  setOrgWorkflowSteps: (_steps: OrgWorkflowStep[]) => {},
  setCurrentStep: (_step: string) => {}, //what the user is looking at
  firstStepIndex: -1,
  setFirstStepIndex: (_step: number) => {},
  index: 0, //row index?
  mediafileId: '', //This is the latest vernacular
  selected: '',
  setSelected: (
    _selected: string,
    _inPlayer: PlayInPlayer,
    _rowData?: IRow[]
  ) => {},
  setMediaSelected: (_id: string, _start: number, _end: number) => {},
  ...
  setItemPlaying: (_playing: boolean) => {},
  playItem: '', //resource
  setPlayItem: (_item: string) => {},
  commentPlaying: false,
  setCommentPlaying: (_playing: boolean, _ended?: boolean) => {},
  commentPlayId: '',
  setCommentPlayId: (_mediaId: string) => {},
  ...
  setPlayerSegments: (_segments: string) => {},
  recording: false,
  ...
  setDiscussionMarkers: (_markers: IMarker[]) => {},
  handleHighlightDiscussion: (_time: number | undefined) => {},
  highlightDiscussion: undefined as number | undefined,
  refresh: 0,
  prjId: '',
  forceRefresh: (_rowData?: IRow[]) => {},
...
const PassageDetailContextMobile = React.createContext({} as IContext);

interface IProps {
  children: React.ReactElement;
}
const PassageDetailProviderMobile = (props: IProps) => {
  ...
  const handleSetCurrentStep = (stepId: string) => {
    ...
    if (step && tool !== ToolSlug.Resource && tool !== ToolSlug.Transcribe) {
      ...
      if (
        ...
        state.rowData[0] !== undefined &&
        state.rowData[0].isVernacular
      ) {
  ...
  const setStepComplete = async (
    ...
    const remId =
      remoteId('orgworkflowstep', stepid, memory?.keyMap as RecordKeyMap) ||
      stepid;
    const step = completed.find((s) => s.stepid === remId);
    ...
    try {
      ...
      logError(
        Severity.error,
        errorReporter,
        infoMsg(err as Error, 'Json conversion error')
      );
    }
  ...
  const setStepCompleteTo = async (stepid: string) => {
    if (stepid === '') return;
    const completed: StepComplete[] = [];
    const steps = state.orgWorkflowSteps.sort(
  ...
  const setSelected = (
    ...
    if (inPlayer === PlayInPlayer.yes) {
      ...
      const rows = newRows.length > 0 ? newRows : rowData;
      setState((state: ICtxState) => {
        return {
          ...state,
          ...
          index: i,
          selected,
          playerMediafile: (rows[i] as IRow).mediafile,
          playing: false,
  ...
  useEffect(() => {
    ...
    if (p) {
      ...
      const s = sections.find((s) => s.id === related(p, 'section'));
      if (s) {
  ...
  return (
    <PassageDetailContextMobile.Provider
      value={{
        state: {
          ...state,
          canPublish,
...
export { PassageDetailContextMobile, PassageDetailProviderMobile };
