...
import { asString, scopeI } from './scopeI';
...
export default function FindBibleBrain({
  ...
  useEffect(() => {
    setBibleOpt(null);
    if (lang === null) return;
    const l = languages.find((v) => v.attributes.iso === lang.value);
    if (l) {
      setQueryBible(true);
      ...
      if (response.status === HttpStatusCode.Ok) {
        const getCount = async (resolve: (value?: unknown) => void) => {
          ...
          if (cntresp === 0) {
            ...
            const p = ((total - cntresp) / total) * 100;
            setProgress(p);
            setCount(cntresp);
          }
        };
