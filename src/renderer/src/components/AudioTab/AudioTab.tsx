...
export function AudioTab() {
  ...
  const hasPassage = (pRow: number) => {
    for (const mediaId of Object.keys(attachMap)) {
  ...
  const handleSave = async (argMap?: IAttachMap) => {
    ...
    let n = 0;
    setComplete(n);
    for (const mediaId of Object.keys(map)) {
  ...
  const doDetach = (mediaId: string) => {
    ...
    if (Object.hasOwn(attachMap, mediaId)) {
  ...
  const doAttach = (mRow: number, pRow: number) => {
    if (Object.hasOwn(attachMap, data[mRow].id)) {
