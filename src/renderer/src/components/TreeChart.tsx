...
const barSeriesForTask = (planwork: any) => {
  const acc: any[] = [];
  const names: string[] = [];
  planwork.forEach((pw: { [key: string]: any }) => {
...
const gridDetailContainerBase = (
  ...
  const GridDetailComponent = ({ row }: any) => {
  ...
  GridDetailComponent.displayName = 'GridDetailComponent';
  return GridDetailComponent;
};

const gridDetailContainer = (t: ITreeChartStrings, data1: any, data2: any) =>
  gridDetailContainerBase(t, data1, data2);

const NoDataCell = ({ style, ...restProps }: any) => {
