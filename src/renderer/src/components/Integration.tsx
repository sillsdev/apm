...
export function IntegrationPanel(props: IProps) {
  ...
  const getUserName = (token: string, errorReporter: any, pendingmsg: string) =>
    dispatch(actions.getUserName(token, errorReporter, pendingmsg) as any);
  const getProjects = (
    token: string,
    pendingmsg: string,
    errorReporter: any,
    ...
    dispatch(
      actions.getProjects(token, pendingmsg, errorReporter, languageTag) as any
    );
  const getLocalProjects = (
    ...
    dispatch(
      actions.getLocalProjects(ptPath, pendingmsg, projIds, languageTag) as any
    );
  const syncProject = (
    token: string,
    projectId: number,
  ...
  const resetSync = () => dispatch(actions.resetSync() as any);
  const resetCount = () => dispatch(actions.resetCount() as any);
  const resetProjects = () => dispatch(actions.resetProjects() as any);
  const resetUserName = () => dispatch(actions.resetUserName() as any);
  ...
  const [exportNumbers, setExportNumbers] = useState(
    JSON.parse(
      (getProjectDefault(projDefExportNumbers) as string) ?? 'false'
    ) as boolean
  );
  ...
  const addParatextIntegration = async (local: string): Promise<string> => {
    ...
    const rec = rn.normalizeRecord(int);
  ...
  const handleParatextProjectChange = (e: any) => {
    const index: number = paratext_projects.findIndex(
    ...
    if (index >= 0) {
      ...
      const projint = getProjectIntegration(paratextIntegration);
  ...
  const formatWithLanguage = (replLang: string): string => {
    ...
    const language =
      proj && proj.attributes ? proj.attributes.languageName : '';
  ...
  useEffect(() => {
    if (local) {
      getParatextDataPath().then((val) => setPtPath(val as string));
    } else {
    ...
    if (!busy) {
      if (!paratext_projectsStatus) {
        ...
        if (local) {
          ...
          const projIds = localprojs.map((pi) => {
            const settings = JSON.parse(pi.attributes.settings);
            return {
              Name: settings.Name,
              Id:
                remoteId(
