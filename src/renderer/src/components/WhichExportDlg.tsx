...
interface IWhichExportProps {
  ...
  doProjectExport: (exportType: ExportType, importedDate?: DateTime) => void;
}
...
export const WhichExportDlg = ({
  ...
  const doITF = (event: React.MouseEvent<HTMLElement>) => {
    if (event.shiftKey) {
      ...
      const oldDate = DateTime.fromISO(op.attributes.snapshotDate);
  ...
  const doITFwDate = () => {
    const doIt = (newDate: DateTime | undefined) => {
      setOpenDatePicker(false);
      setOpenExport(false);
      doProjectExport(ExportType.ITF, newDate);
    };
    const newDate = DateTime.fromISO(sinceDate);
    if (newDate.isValid) {
      doIt(!newDate.equals(snapshotDate as DateTime) ? newDate : undefined);
    } else {
