...
interface ReferenceTableProps {
  bookData: BookRef[];
  onCommit?: ((refs: BookRef[]) => void) | undefined;
  onCancel: () => void;
}
...
export default function ReferenceTable({
  ...
  const deleteConfirmed = () => {
    ...
    for (let i = 0; i < myData.length; i++) {
      if (i < confirmRow) {
        newData.push(myData[i] as ICell[]);
        c++;
        ...
        newData.push(
          rowCells(
            c,
            (myData[i] as ICell[]).map((v) => v.value)
          )
        );
        c++;
      }
  ...
  const handleSave = () => {
    ...
    dataRef.current.forEach((r, i) => {
      const book = r[ColName.Book] as ICell;
      const refs = r[ColName.Refs] as ICell;
      if (
        (i < last && !book.value) ||
        (i > 0 && refs.value && !refTest(refs.value))
      ) {
        error++;
      } else if (i > 0 && book.value) {
        refData.push({
          code: book.value,
          refs: refs.value,
        });
  ...
  const handleCellsChanged = (changes: Array<ICellChange>) => {
    ...
    changes.forEach((c) => {
      ...
      ((newData[c.row] as ICell[])[c.col] as ICell).value = c.value;
      if (bookCng || c.col === ColName.Refs)
        // Turns error background on and off
        newData[c.row] = rowCells(
          c.row,
          (newData[c.row] as ICell[]).map((i) => i.value)
        );
    });
    const len = dataRef.current.length;
    const lastRow = dataRef.current[len - 1] as ICell[];
    if ((lastRow[0] as ICell).value || (lastRow[1] as ICell).value)
  ...
  const confirmMsg = (row: number | undefined) => {
    if (!row) return '';
    const bookCode = ((dataRef.current[row] as ICell[])[ColName.Book] as ICell)
      .value;
    let bookLabel = '';
    if (suggestionRef.current) {
      ...
      if (bookLabels.length > 0)
        bookLabel = (bookLabels[0] as OptionType).label;
    }
    return `${bookLabel} ${((dataRef.current[row] as ICell[])[ColName.Refs] as ICell).value}`;
  };
    ...
    <Stack spacing={1}>
      <Content>
        <DataSheet
          ...
          {...(onCommit && { onCellsChanged: handleCellsChanged })}
        />
      </Content>
      <ActionRow>
