...
import PassageDataTable, { IRRow } from '../PassageDataTable';
...
export const SelectNote = (props: IProps) => {
      ...
      rangeList.forEach((r) => {
        ...
        if (m) {
          const chapRefs = bookRefs.filter(
            (r) => r.attributes.chapter === parseInt(m[1] || '1')
          );
          chapRefs.forEach((c) => addRes(chapSet, c));
          const startVerse = parseInt(m[2] || '1');
          const endVerse = m[3] ? parseInt(m[3] || '1') : startVerse;
  ...
  const termsOfUse = (i: number): string | undefined => {
    return (notes[i] as SharedResourceD).attributes.termsOfUse;
  };
