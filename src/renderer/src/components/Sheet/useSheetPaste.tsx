...
export const useWfPaste = (props: IProps) => {
  ...
  const validTable = (rows: string[][]) => {
    ...
    if (!flat) {
      ...
      const firstSection = rows.findIndex((r) => isNumPat.test(r[secNumCol]));
      const firstPassage = rows.findIndex((r) => isNumPat.test(r[passNumCol]));
      ...
      rows
        ...
        .forEach((r) => {
          ...
          if (ws.passageSeq) {
            ...
            ws.published = [] as PublishDestinationEnum[];
