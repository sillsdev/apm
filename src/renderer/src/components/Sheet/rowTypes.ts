...
export const rowTypes = (rowInfo: ISheet[]) => {
  const isSectionType = (i: number) =>
    i >= 0 && i < rowInfo.length ? isSectionRow(rowInfo[i] as ISheet) : false;
  const isSectionHead = (i: number) =>
    isSectionType(i)
      ? (rowInfo[i] as ISheet).level === SheetLevel.Section
      : false;
  const isPassageType = (i: number) =>
    i >= 0 && i < rowInfo.length ? isPassageRow(rowInfo[i] as ISheet) : false;
  const isVerseRange = (i: number) =>
    isPassageType(i)
      ? (rowInfo[i] as ISheet).passageType === PassageTypeEnum.PASSAGE
      : false;
  const isPassageOrNote = (i: number) =>
    isPassageType(i) &&
    [PassageTypeEnum.PASSAGE, PassageTypeEnum.NOTE].includes(
      (rowInfo[i] as ISheet).passageType
    );
  ...
  const isMovement = (i: number) =>
    i >= 0 &&
    i < rowInfo.length &&
    (rowInfo[i] as ISheet).level === SheetLevel.Movement;
  const isBeta = (i: number) =>
    i >= 0 &&
    i < rowInfo.length &&
    (rowInfo[i] as ISheet).published?.includes(PublishDestinationEnum.AkuoBeta);
