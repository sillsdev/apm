...
export const usePlanSheetFill = ({
  ...
  const stepCell = ({
    ...
    ({
      value: passage &&
        refCol > 0 &&
        !isPublishingTitle(
          (row[refCol] as string).toString(),
          inlinePassages
        ) && (
          <Badge
            badgeContent={(rowInfo[rowIndex] as ISheet).discussionCount}
            color="secondary"
          >
            <StageReport
              onClick={readonly ? undefined : handlePassageDetail(rowIndex)}
              step={(rowInfo[rowIndex] as ISheet).step || ''}
              tip={tv.gotowork}
            />
          </Badge>
        ),
  ...
  const assignmentCell = (rowIndex: number, calcClassName: string) =>
    ({
      value: (rowInfo[rowIndex] as ISheet).assign ? (
        <TaskAvatar assigned={(rowInfo[rowIndex] as ISheet).assign || null} />
      ) : (
        <Typography
          sx={{
            maxWidth: '80px',
        ...
        >
          {schemeName((rowInfo[rowIndex] as ISheet).scheme?.id || '')}
        </Typography>
      ),
      readOnly: true,
      className: calcClassName,
  ...
  const publishedValue = ({
    ...
    return (
      <PlanPublishActions
        rowIndex={rowIndex}
        isPassage={passage}
        publishStatus={(rowInfo[rowIndex] as ISheet).publishStatus || ''}
        mediaId={(rowInfo[rowIndex] as ISheet).mediaId?.id || ''}
        mediaShared={(rowInfo[rowIndex] as ISheet).mediaShared}
        canEdit={canEdit}
        onHistory={onHistory}
      />
    );
    ...
    return (
      <PlanAudioActions
        ...
        onEdit={
          ...
          ((rowInfo[rowIndex] as ISheet).passageType === PassageTypeEnum.NOTE ||
            (shared &&
              (rowInfo[rowIndex] as ISheet).passageType !==
                PassageTypeEnum.CHAPTERNUMBER))
            ? onEdit
            : undefined
        }
        isPlaying={
          srcMediaId === (rowInfo[rowIndex] as ISheet).mediaId?.id &&
          mediaPlaying
        }
        sharedResource={(rowInfo[rowIndex] as ISheet).sharedResource}
      />
    );
  };
      ...
      <>
        ...
        {!isNote && (
          <TitleEdit
            ...
            passageId={
              (rowInfo[rowIndex] as ISheet).kind === IwsKind.Passage
  ...
  const refValue = (e: string | number) => {
    const pt = passageTypeFromRef(e as string, inlinePassages);
    if (pt !== PassageTypeEnum.PASSAGE)
      return <RefRender value={e as string} pt={pt} />;
    return e;
  };
  ...
  const graphicValue = (rowIndex: number, readonly: boolean) => {
    if (
      ...
      ![PassageTypeEnum.NOTE, PassageTypeEnum.CHAPTERNUMBER].includes(
        passageTypeFromRef(
          (rowInfo[rowIndex] as ISheet).reference,
          inlinePassages
        )
      )
    ) {
    ...
    if ((rowInfo[rowIndex] as ISheet).graphicUri) {
      return (
        <Avatar
          sx={{ ...pointer, ...border }}
          src={(rowInfo[rowIndex] as ISheet).graphicUri}
          variant="rounded"
    ...
    return (
      <Avatar
        {...stringAvatar(
          (rowInfo[rowIndex] as ISheet).reference ||
            `${organizedBy} ${(rowInfo[rowIndex] as ISheet).sectionSeq}`,
          { ...pointer, ...border }
        )}
        variant="rounded"
  ...
  const rowCells =
    ...
    (e: string | number, cellIndex: number) => {
      ...
      if (
        /CHNUM/.test((rowData[rowIndex] as IRow)[refCol] as string) &&
        !hidePublishing &&
        publishingOn
      ) {
        if (cellIndex === titleCol) {
          return {
            ...
            component: TitleValue(
              (rowData[rowIndex] as IRow)[descCol] as string,
              rowIndex,
              descCol,
              !canEditTitle
            ),
      ...
      if (
        /NOTE/.test((rowData[rowIndex] as IRow)[refCol] as string) &&
        !hidePublishing &&
        publishingOn &&
        cellIndex === titleCol
      ) {
        return {
          value: e,
          component: TitleValue(
            (rowInfo[rowIndex] as ISheet).sharedResource?.attributes.title ||
              '',
            rowIndex,
            cellIndex,
            !canEditTitle,
      ...
      : ({
          value: (
            <PlanActionMenu
              ...
              psgType={(rowInfo[rowIndex] as ISheet).passageType}
              organizedBy={organizedBy}
    ...
    (row: IRow, rowIndex: number) => {
      ...
      sheetRow.push(
        actionCell({
          ...
          canPlay:
            !anyRecording &&
            ((rowInfo[rowIndex] as ISheet).mediaId?.id || '') !== '',
