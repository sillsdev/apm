...
export function ScriptureTable(props: IProps) {
  ...
  const getGraphicPickerRefString = (i: number) => {
    const { ws } = getByIndex(sheetRef.current, i);
    if (
      passageTypeFromRef(ws?.reference ?? '', flat) === PassageTypeEnum.NOTE
    ) {
      if (ws?.passage) {
        const noteRef = computeRef(curNoteRef(ws?.passage ?? ''));
        if (noteRef.length > 0) return noteRef;
      }
    } else if (
      passageTypeFromRef(ws?.reference ?? '', flat) === PassageTypeEnum.MOVEMENT
    ) {
      ...
      passageTypeFromRef(ws?.reference ?? '', flat) ===
      ...
      return firstBook + ' ' + chapter + ':1-999';
    } else if (
      [PassageTypeEnum.BOOK, PassageTypeEnum.ALTBOOK].includes(
        passageTypeFromRef(ws?.reference ?? '', flat)
      )
    ) {
      const verses = engVrs.get(firstBook);
