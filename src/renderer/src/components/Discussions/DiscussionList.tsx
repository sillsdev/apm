...
export function DiscussionList() {
  ...
  useEffect(() => {
    const def = getOrgDefault(orgDefaultDiscussionFilter) as IFilterState;
    if (def) {
      setFilterStatex(def);
  ...
  const discussionSort = (x: Discussion, y: Discussion) => {
    const topicSort = () => {
      const xreg = DiscussionRegion(x);
      const yreg = DiscussionRegion(y);
      return xreg && yreg
  ...
  const doTheThing = () => {
    ...
    if (confirmAction.startsWith(WaitSave.filter)) {
      const what = confirmAction.substring(WaitSave.filter.length);
