...
export const DiscussionCard = (props: IProps) => {
  ...
  useEffect(() => {
    ...
    if (Boolean(discussion.attributes?.subject) && !discussion.id) {
  ...
  const myCommentIds = useMemo(() => {
    const myIds = myComments.map((d) => d.id);
  ...
  useEffect(() => {
    ...
    if (!changeRef.current) {
      const anyChanged = Object.keys(toolsChanged).some(
        (t) =>
          myCommentIds.includes(t) &&
          !(toolsChanged[t] as SaveInfo).clearChanged
      );
      if (anyChanged)
  ...
  useEffect(() => {
    if (artifactcategorys && discussion.relationships?.artifactCategory) {
      const u = artifactcategorys.filter(
        (u) => related(discussion, 'artifactCategory') === u.id
      ) as ArtifactCategory[];
      if (u.length > 0) {
        setArtifactCategory(
          localizedArtifactCategory(
            (u[0] as ArtifactCategory).attributes.categoryname
          )
        );
        return;
      }
  ...
  function selectDiscussion(discussion: Discussion) {
    console.log('selectDiscussion not implemented', discussion);
  }
  ...
  useEffect(() => {
    if (assignedToMeRef.current) {
      const sortedByUpdatedDesc = [...myComments];
      ...
      if (
        ...
        if (hasPermission(PermissionName.CIT) && mentorGroup) {
          ...
          for (
            ...
            CommentAuthor(sortedByUpdatedDesc[ix] as CommentD) === user;
            ix++
          );
          if (ix < sortedByUpdatedDesc.length) {
            handleUserChange(
              CommentAuthor(sortedByUpdatedDesc[ix] as CommentD)
            );
            setChangeAssignment(false);
          }
        }
