...
export function ImportTab(props: IProps) {
    ...
    switch (err.errStatus) {
      ...
      case 422:
        ...
        if (Array.isArray(json)) {
          let msg = '';
          json.forEach((fr) => {
            const thiserr = errorStatus(fr.Status, fr.Message);
  ...
  const sectionFromPassage = (passage: Passage, remote: boolean) => {
    const sectionid = passage.relationships?.section?.data as RecordIdentity;
    if (sectionid) {
  ...
  const planFromSection = (section: Section, remote: boolean) => {
    const planid = section.relationships?.plan?.data as RecordIdentity;
    if (planid) {
      return memory.cache.query((q) =>
        q.findRecord({
          type: 'plan',
      ...
      changeReport.forEach((c: IChanges) => {
        ...
        switch (c?.type) {
          ...
          case 'mediafile':
            ...
            const online = c.online.data as MediaFile;
            if (
          ...
          case 'section':
            ...
            if (section) {
              ...
              if (
                ...
                const editor = section?.relationships?.editor?.data
                  ...
                  : undefined;
                const oldeditor = memory.cache.query((q) =>
                  q.findRecord({
                    type: 'user',
                    id: remoteIdGuid(
                      'user',
              ...
              if (
                ...
                const transcriber = section.relationships?.transcriber?.data
                  ? (memory.cache.query((q) =>
                      q.findRecord({
                        type: 'user',
                        id: remoteIdGuid(
                  ...
                  : undefined;
                const oldtranscriber = memory.cache.query((q) =>
                  q.findRecord({
                    type: 'user',
                    id: remoteIdGuid(
                      'user',
          ...
          case 'user':
            ...
            const olduser = c.online.data as User;
