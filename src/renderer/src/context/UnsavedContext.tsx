...
export interface IIndexableSaveInfo<SaveInfo> {
  [key: string]: SaveInfo;
}
...
const UnsavedProvider = (props: PropsWithChildren) => {
  ...
  const anySaving = (myToolId?: string) => {
    ...
    const saving = Object.keys(toolsChangedRef.current).reduce(reducer, '');
  ...
  const saveRequested = (toolId: string) => {
    return toolsChangedRef.current[toolId]?.startSave || false;
  };
  const clearRequested = (toolId: string) => {
    return toolsChangedRef.current[toolId]?.clearChanged || false;
  };
  ...
  const toolChanged = (
    ...
    if (setit) {
      setToolsChanged({ ...toolsChangedRef.current });
      const anyChanged = Object.keys(toolsChangedRef.current).length > 0;
