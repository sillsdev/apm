...
**Segment boundaries**:
The time ranges that divide the vernacular **source recording** into **segments** for **Phrase Back Translation** and related `phraseBackTranslate` steps. Stored on the vernacular media under **`BT`** (`NamedRegions.BackTranslation`) — the canonical copy for non-BOLD phrase-level back translation. On first entry with no PBTs, when `BT` is empty or identical to **Mark Verses** marks, boundaries are created by auto-segment **constrained by Mark Verses** (verse edges kept; long verses split). **More Segments** / **Less Segments** keep that verse constraint. Manual **+ / −** (playhead) and recording-pass **Split Segment** / **Combine with Next Segment** also edit boundaries. **PBT** and **Retell** recordings link to a segment by matching its time range via `sourceSegments`, not by storing their own boundaries.
_Avoid_: Clause boundaries (BOLD **`clause`** bucket); treating Mark Verses as an unremovable lock on manual −; boundaries stored only on back-translation artifacts

**Reset** (Phrase Back Translation):
Recording-pass control that restores **segment boundaries** to the map captured when the step opened (after Verse-constrained seed or load of existing `BT`), deletes all PBTs for the passage after confirm when any exist, clears the undo stack (not undoable), and returns the user to the **listen pass**. Confirm also appears when boundaries drifted with no recordings yet. Not shown on the listen pass. Distinct from Transcribe Reset (restore Verse marks as the transcription region).
_Avoid_: Careful Speech; Mark Verses–only restore without the step baseline; undoable Reset that resurrects deleted audio

**Retell Back Translation** (step):
A non-BOLD `phraseBackTranslate` step configured with the **Retell** artifact where the user retells the entire passage for the consultant in one guided **listen pass** / **recording pass** — exactly **one segment** covering full vernacular audio, with no More/Fewer/Split/Combine or player boundary tools. Legacy recordings with empty `sourceSegments` still count as complete for that segment.
