...
A separate non-BOLD workflow step (process preset / named step, not chosen via the Phrase BT settings artifact dropdown) that uses the shared `phraseBackTranslate` tool with the **Retell** artifact. The user retells the entire passage in one guided **listen pass** / **recording pass** — exactly **one segment** covering full vernacular audio, with **no segmenting** (no More/Fewer/Split/Combine or player boundary tools). Language is normally the **vernacular**; teams typically want **multiple speakers**. The full-span segment is **ephemeral** (synthesized from audio duration at runtime) — Retell never reads or writes vernacular named-region segment maps, including legacy **`BT`** or Phrase BT language buckets. Step language is still required, unique among Retell steps, stamped on takes, and used for language-scoped matching. Fully isolated from Phrase Back Translation: Retell actions never change Phrase BT PBTs or boundaries, and Phrase BT **Reset** never changes Retell takes. Legacy Retell recordings with empty `sourceSegments` still count as complete for that single segment.
_Avoid_: Community Testing Retell; offering Retell as an artifact option on Phrase BT step settings; multi-segment Retell BT; Careful Speech; persisting Retell boundaries on vernacular media; sharing Phrase BT `BT`/language boundary buckets; any Reset or resegment on Retell that mutates Phrase BT state; assuming one speaker or an LWC for Retell

**Phrase Back Translation** (step):
A non-BOLD workflow step where the user listens to full-speed **vernacular** by phrase and records the meaning as a **PBT** in **LWC** — one workflow step with no separate **Careful Speech** artifact step (e.g. Kom directly to English). Uses the **listen pass** / **recording pass** guided interaction via `PassageDetailPhraseBackTranslate` and the `phraseBackTranslate` tool. Shares the guided shell with Careful Speech but diverges on boundary chrome (player +/−/Undo, recording-pass Reset to step baseline, sequential arrows around Record, verse-constrained auto-segment). Typically **one speaker** and an **LWC** (not vernacular). **User-facing name is Phrase Back Translation, not Careful Speech**. Step settings configure language only — not artifact type (Retell is a different step).
_Avoid_: Careful Speech (name or step outside BOLD); LWC Translation (BOLD step); Whole Back Translation; Community Test Retell in Phrase BT settings; calling the output careful speech; legacy single-phase-only UX

**Phrase Back Translate** (tool):
Developer-facing tool slug for phrase-level listen-and-record steps on non-BOLD processes (**Phrase Back Translation** and **Retell Back Translation**) and for **LWC Translation** in BOLD (PBT artifact, LWC audio). Artifact type is fixed by the workflow step preset (PBT vs Retell), not by a settings dropdown on the Phrase BT dialog. BOLD **Careful Speech** uses the separate `carefulSpeech` tool and **Careful Speech artifact** type — that tool and step name appear only in BOLD workflows.
_Avoid_: Careful Speech (user-facing name outside BOLD); LWC Translation (user-facing BOLD step name); Phrase Back Translate (in BOLD Careful Speech UI copy); choosing Retell vs PBT via Phrase BT settings artifact dropdown
...
**StepEditor**:
Team-level admin UI for configuring a team's workflow — step sequence, tools, and per-step settings (e.g. language on Phrase BT / Retell BT; artifact type on Transcribe / Paratext). **Phrase Back Translate** step settings configure language only; Retell is not offered as an artifact choice there.
_Avoid_: Workflow Navigation; Assignments tab; Community Test Retell in Phrase BT settings
