...
function findBurritoZipEntry(entries, ingredientKey) {
...
function buildTimingSegmentMap(zipEntries, ingredients) {
  ...
  for (const [key, ingredient] of Object.entries(ingredients ?? {})) {
    ...
    const entry = findBurritoZipEntry(allEntries, key);
...
async function transformBurritoToPTF() {
  ...
  for (const zipFile of zipFiles) {
    ...
    for (const [key, ingredient] of audioIngredients) {
      const zipEntry = findBurritoZipEntry(entries, key);
      if (!zipEntry) {
        console.warn(
    ...
    const sortedBookGroups = Array.from(bookGroups.entries()).sort(
      ([a], [b]) => {
    ...
    const emitBookPtf = async (bookGroupCode, bookAudioEntries) => {
      ...
      const bookSuffix = isMultiBook
        ...
        : '';
      const scopedProjectName = `${projectName}${bookSuffix}`;
      const scopedPlanName = `${planName}${bookSuffix}`;
      ...
      const organization = createJsonApiRecord(
        'organizations',
        {
          ...
          slug: sanitizeFilename(scopedProjectName, 'burrito'),
      ...
      bookAudioEntries.forEach((audioEntry, index) => {
